...
const Landing = ({ onEnter }) => {
  ...
  const theme = {
    bg: dark ? '#0A0B0F' : '#f8f8ff',
    card: dark ? '#111318' : '#ffffff',
    border: dark ? '#1E2130' : '#e2e2f0',
    text: dark ? '#E8EAF2' : '#0f0f1a',
    sub: dark ? '#8890AA' : '#666680',
    muted: dark ? '#5A6180' : '#aaaacc',
    nav: dark ? '#0A0B0Fcc' : '#f8f8ffcc',
  };

  const features = [
    { icon: '⇄', title: 'Bridge', desc: 'Cross-chain USDC transfers powered by Circle CCTP V2. Fast, secure, trustless.', color: '#00E5A0' },
    { icon: '↺', title: 'Swap', desc: 'Seamlessly swap USDC and EURC on Arc Testnet with real-time rates.', color: '#00E5A0' },
    { icon: '🛡️', title: 'RefundPay', desc: 'On-chain escrow payments with dispute resolution. Send crypto with chargeback protection.', color: '#FF6B35' },
    { icon: '◈', title: 'Dashboard', desc: 'Track your balances across Arc and Sepolia networks in one place.', color: '#00E5A0' },
    { icon: '◷', title: 'History', desc: 'Full transaction history with real-time updates via Arc API.', color: '#FF6B35' },
  ];
    ...
    <div style={{
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontWeight: '800', fontSize: '18px', color: theme.text }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button onClick={() => onEnter(dark)} style={{
            background: '#00E5A0',
            color: '#0A0B0F', border: 'none', padding: '10px 24px',
      ...
      <div style={{
        ...
        background: dark
          ? 'radial-gradient(ellipse at 50% 0%, #00E5A015 0%, transparent 70%)'
          : 'radial-gradient(ellipse at 50% 0%, #00E5A008 0%, transparent 70%)',
      }}>
        <div style={{
          display: 'inline-block', padding: '6px 16px', borderRadius: '20px',
          background: 'rgba(0,229,160,0.08)', border: '1px solid rgba(0,229,160,0.2)',
          color: '#00E5A0', fontSize: '13px', fontWeight: '600', marginBottom: '24px',
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => onEnter(dark)} style={{
            background: '#00E5A0',
            color: '#0A0B0F', border: 'none', padding: '16px 36px',
      ...
      <div style={{
        ...
        {[
          ...
          <div key={i} style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '32px', fontWeight: '800', color: '#00E5A0', fontFamily: 'monospace' }}>{s.value}</div>
            <div style={{ fontSize: '14px', color: theme.text, fontWeight: '600' }}>{s.label}</div>
            <div style={{ fontSize: '12px', color: theme.muted, fontFamily: 'monospace' }}>{s.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        background: dark
          ? 'radial-gradient(ellipse at 50% 100%, #00E5A015 0%, transparent 70%)'
          : 'radial-gradient(ellipse at 50% 100%, #00E5A008 0%, transparent 70%)',
        borderTop: '1px solid ' + theme.border,
      }}>
        ...
        <button onClick={() => onEnter(dark)} style={{
          background: '#00E5A0',
          color: '#0A0B0F', border: 'none', padding: '18px 48px',
